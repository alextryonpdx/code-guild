...
# ToDo line 193 has no attribute to return to project_list_menu
# ToDo not sure that items on projects are being saved, no files created.
#    ToDo: could be because the cat.ed name pair doesn't have str attribute
# did not push to github. analyze changes in the morning
# maybe re-format pickle method
...
"""
#ToDo "projects" becomes a str type at some point
    #Todo it makes it impossible to return to project list menu
    def add_item(self, username, projects, project_name):
    #    print projects
        ...
        if add_on == 1:
            ...
            # create event with title "%s: %s is Due" %(project: task)
            # give option to mark task as complete, run method in background
            print "%s: due %s" % (task, due_date)
            new_task = ProjectItem(task, due_date, complete, *contacts)
            self.args.append(new_task)
            self.pickle_project(project_name, projects)
            self.project_menu(username, projects, project_name)
        if add_on == 2:
    ...
    # deletes list item
    def delete_item(self, username, projects, project_name):
        self.__str__()
    ...
    def edit_item(self, username, projects, project_name):
        self.pickle_project(project_name, projects)
        self.__str__()
        ...
        if item_edit == 2:
            ...
            self.project_menu(username, projects, project_name)
        if item_edit == 3:
            self.args[to_edit - 1].complete = True
            self.pickle_project(project_name, projects)
            self.project_menu(username, projects, project_name)
        if item_edit == 4:
            print "1. add contact \n2. remove contact\n3. cancel"
            ...
            if contact_to_do == 2:
                ...
                self.pickle_project(project_name, projects)
            if contact_to_do == 3:
                self.edit_item(username, projects, project_name)
        if item_edit == 5:
            self.pickle_project(project_name, projects)
            self.project_menu(username, projects, project_name)
    ...
    def project_menu(self, username, projects, project_name):
        project_menu_choice = 0
    #    print projects.username
        self.unpickle_project(project_name, projects)
        ...
        if project_menu_choice == 1:
            self.add_item(username, projects, project_name)
        if project_menu_choice == 2:
            self.edit_item(username, projects, project_name)
        if project_menu_choice == 3:
            self.delete_item(username, projects, project_name)
        if project_menu_choice == 4:
            self.__str__()
            menu = raw_input("press enter to continue")
            self.project_menu(username, projects, project_name)
        if project_menu_choice == 5:
            username.project_list_menu(self.username, projects)
        if project_menu_choice in range(0, 6) is False:
            print "you made an improper selection"
            self.project_menu(username, projects, project_name)

    def pickle_project(self, project_name, projects):
        project_file = self.project_name
        pickle.dump(self.args, open("p%s.pk1" % project_file, "wb"))

    def unpickle_project(self, project_name, projects):
        project_file = self.project_name
        self.args = pickle.load(open("p%s.pk1" % project_file, "rb"))
...
# todo                           down menu chain to allow for return
    ...
    def project_list_menu(self, username, projects):
        #self.unpickle_projects(username)
        print "1. new project"
    ...
    def pickle_projects(self, username):
        projects_file = username
        pickle.dump(self.args, open("p_%s.pk1" % projects_file, "wb"))

    def unpickle_projects(self, username):
        projects_file = username
        self.args = pickle.load(open("p_%s.pk1" % projects_file, "rb"))
